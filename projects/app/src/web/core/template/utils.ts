import { getFileViewUrl, postChunks2Template } from '@/web/core/template/api';
import { TrainingModeEnum } from '@fastgpt/global/core/template/constants';
import { delay } from '@fastgpt/global/common/system/utils';
import { strIsLink } from '@fastgpt/global/common/string/tools';
import type {
  FileCreateTemplateCollectionParams,
...
export const fileCollectionCreate = ({
  ...
  data: FileCreateTemplateCollectionParams;
  percentListen: (percent: number) => void;
}) => {
  const form = new FormData();
  form.append('data', JSON.stringify(data));
  form.append('metadata', JSON.stringify(metadata));
  form.append('bucketName', BucketNameEnum.template);
  form.append('file', file, encodeURIComponent(file.name));

  return POST<string>(`/core/template/collection/create/file?templateId=${data.templateId}`, form, {
    timeout: 480000,
    onUploadProgress: (e) => {
      if (!e.total) return;
...
export async function chunksUpload({
  ...
  async function upload(data: PushTemplateDataChunkProps[]) {
    return postChunks2Template({
      collectionId,
      trainingMode,
      data,
      prompt,
