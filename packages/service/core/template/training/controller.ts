...
import { MongoTemplateTraining } from './schema';
import type {
  PushTemplateDataChunkProps,
  PushTemplateDataProps,
  PushTemplateDataResponse
} from '@fastgpt/global/core/template/api.d';
import { getCollectionWithTemplate } from '../controller';
...
export const lockTrainingDataByTeamId = async (teamId: string, retry = 3): Promise<any> => {
  try {
    await MongoTemplateTraining.updateMany(
      {
        teamId
      },
      {
...
export async function pushDataListToTrainingQueue({
  ...
  templateModelList: LLMModelItemType[];
} & PushTemplateDataProps): Promise<PushTemplateDataResponse> {
  const {
    templateId: { _id: templateId, vectorModel, agentModel }
  } = await getCollectionWithTemplate(collectionId);
  ...
  const checkModelValid = async ({ collectionId }: { collectionId: string }) => {
    ...
    if (trainingMode === TrainingModeEnum.qa) {
      const qaModelData = templateModelList?.find((item) => item.model === agentModel);
      if (!qaModelData) {
  ...
  const set = new Set();
  const filterResult: Record<string, PushTemplateDataChunkProps[]> = {
    success: [],
    overToken: [],
    repeat: [],
    error: []
  ...
  // insert data to db
  const insertData = async (dataList: PushTemplateDataChunkProps[], retry = 3): Promise<number> => {
    try {
      const results = await MongoTemplateTraining.insertMany(
        dataList.map((item, i) => ({
          teamId,
          tmbId,
          templateId,
