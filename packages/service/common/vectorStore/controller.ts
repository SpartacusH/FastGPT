...
export const deleteTemplateDataVector = getVectorObj().delete;
...
export const insertTemplateDataVector = async ({
  model,
  query,
  ...props
}: InsertVectorProps & {
...
export const updateTemplateDataVector = async ({
  id,
  ...props
}: InsertVectorProps & {
  id: string;
  ...
  // insert new vector
  const { charsLength, insertId } = await insertTemplateDataVector(props);
